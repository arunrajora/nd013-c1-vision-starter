...
def process_tfr(path, data_dir):
    ...
    logger.info(f"destination is {dest}/{file_name}")
...
if __name__ == "__main__":
    ...
    with open("filenames.txt", "r") as f:
        filenames = f.read().splitlines()[:100]
    logger.info(
    ...
    workers = [download_and_process.remote(fn, temp_dir, data_dir) for fn in filenames]
    _ = ray.get(workers)
